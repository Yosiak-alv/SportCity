...
//search Handling
watch(form, debounce(() => {
    router.get('/gyms', {search:form.search , trashed:form.trashed}, { preserveState:true , replace:true });
},500));
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Gyms</h2>
        </template>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="p-5 bg">
                        <Table class="rounded-lg bg-gray-700">
                            ...
                            <slot>
                                <tr v-for="gym in props.gyms.data" :key="gym.id" class="hover:bg-gray-500  bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ">
                                        <div v-if="getPermission('show gym')">
                                            <Link class="flex items-center" :href="route('gyms.show',{id: gym.id})">
                                                {{gym.name}}
                                                <svg fill="none" v-if="gym.deleted_at" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400">
                                    ...
                                    <td class="px-6 py-4">
                                        <div v-if="getPermission('show gym')">
                                            <Link  :href="route('gyms.show',{id: gym.id})">
                                                {{gym.email}}
                                            </Link>
                                        </div>
                                        <div v-else>
                                ...
                                <tr v-if="props.gyms.data.length === 0">
                                    <td class="px-6 py-4 font-medium text-gray-200 whitespace-nowrap" colspan="4">No Gyms found.</td>
                                </tr>
                            </slot>
                        </Table>
                    </div>
                    <Paginate :links="props.gyms.links" class="mb-6"/>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
