...
//search Handling
watch(form, debounce(() => {
    router.get('/coaches', {search:form.search , trashed:form.trashed, gym: form.gym}, { preserveState:true , replace:true });
},500));
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Coaches</h2>
        </template>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="p-5 bg">
                        <Table class="rounded-lg bg-gray-700">
                            ...
                            <slot>
                                <tr v-for="coach in props.coaches.data" :key="coach.id" class="hover:bg-gray-500  bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ">
                                        <div v-if="getPermission('show coach')">
                                            <Link class="flex items-center" :href="route('coaches.show',{id: coach.id})">
                                                {{coach.dui}}
                                                <svg fill="none" v-if="coach.deleted_at" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400">
                                    ...
                                    <td class="px-6 py-4">
                                        <div v-if="getPermission('show coach')">
                                            <Link  :href="route('coaches.show',{id: coach.id})">
                                                {{coach.name}}
                                            </Link>
                                        </div>
                                        <div v-else>
                                    ...
                                    <td class="px-6 py-4">
                                        <div v-if="getPermission('show coach')">
                                            <Link :href="route('coaches.show',{id: coach.id})" v-if="getPermission('show coach')">
                                                {{coach.lastname}}
                                            </Link>
                                        </div>
                                        <div v-else>
                                ...
                                <tr v-if="props.coaches.data.length === 0">
                                    <td class="px-6 py-4 font-medium text-gray-200 whitespace-nowrap" colspan="4">No Coaches found.</td>
                                </tr>
                            </slot>
                        </Table>
                    </div>
                    <Paginate :links="props.coaches.links" class="mb-6"/>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
