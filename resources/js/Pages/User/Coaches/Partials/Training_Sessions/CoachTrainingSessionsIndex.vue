...
watch(search, debounce((value) => {
    router.get(route('coaches.show',{id:props.coachId}), {search:value}, { preserveState:true , replace:true ,preserveScroll:true});
},500));
            ...
            <Card v-for="training_session in props.trainingSessions.data" class="text-base p-2">
                <div class="w-full p-2 ">
                    <div class="text-center">
                        <Link as="button" :href="route('coaches.showTrainingSessions', {coach: props.coachId, id:training_session.id})" 
                            class="inline text-4xl hover:underline">
                            {{training_session.id}},{{ training_session.name }},{{ training_session.pivot.coach_id}}
                        </Link>
                    </div>
